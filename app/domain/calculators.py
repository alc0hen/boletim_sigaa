...
class IFAcademicCalculator(AcademicCalculator):
    ...
    def calculate(self, grades: List[Dict[str, Any]]) -> CourseResult:
        ...
        for g in grades:
            ...
            if val is None:
                continue
            if '1' in name and 'Unidade' in name:
                b_grades[0] = val
            elif '2' in name and 'Unidade' in name:
                b_grades[1] = val
            elif '3' in name and 'Unidade' in name:
                b_grades[2] = val
            elif '4' in name and 'Unidade' in name:
                b_grades[3] = val
            elif name == '1':
                b_grades[0] = val
            elif name == '2':
        ...
        details = {'b1': b_grades[0], 'b2': b_grades[1], 'b3': b_grades[2], 'b4': b_grades[3], 'r1': r_grades[0], 'r2': r_grades[1], 'styling': styling}
        return CourseResult(status=status, average=final_avg, needed=needed, details=details, message=f'Falta {needed:.1f} pts' if needed > 0 and status != CourseStatus.FAILED else '', is_critical=status == CourseStatus.FAILED)
...
class UFAcademicCalculator(AcademicCalculator):
    ...
    def calculate(self, grades: List[Dict[str, Any]]) -> CourseResult:
        ...
        details = {'av1': av1, 'av2': av2, 'reav': reav, 'final': final_exam, 'nf': nf}
        return CourseResult(status=status, average=nf, needed=needed, details=details, message=message, is_critical=status == CourseStatus.FAILED)