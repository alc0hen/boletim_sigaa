document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCards() {
        ...
        const oldCards = mobileContainer.querySelectorAll('.ficha-card');
        ...
        pendingReviews.forEach((rev, idx) => {
            ...
            card.innerHTML = `
                ...
                <div class="prof-name">Prof. ${formatProfName(rev.professor)}</div>
                
                <div class="rating-label">Quão exigente foi esta disciplina?</div>
                <div class="rating-group">
                    <button class="btn-rate r1" onclick="rateCard(${idx}, 1, event)">1</button>
                    <button class="btn-rate r2" onclick="rateCard(${idx}, 2, event)">2</button>
                    <button class="btn-rate r3" onclick="rateCard(${idx}, 3, event)">3</button>
                    <button class="btn-rate r4" onclick="rateCard(${idx}, 4, event)">4</button>
                    <button class="btn-rate r5" onclick="rateCard(${idx}, 5, event)">5</button>
                </div>
                
                <button class="btn-skip" onclick="skipCard(${idx})">Pular matéria</button>
            `;
    ...
    function updateProgress() {
        ...
        const pct = ((currentCardIndex) / total) * 100;
        fill.style.width = `${pct}%`;
        text.textContent = `${currentCardIndex} de ${total} avaliadas`;
    ...
    function nextCard(idx, exitClass) {
        ...
        if (currentCardIndex < pendingReviews.length) {
            const nCard = document.getElementById(`card-${currentCardIndex}`);
            if(nCard) nCard.classList.add('active');
