...
def is_configured() -> bool:
    return bool(os.environ.get('SIGAA_API_URL') and os.environ.get('SIGAA_API_PRIVATE_KEY'))
...
class SigaaRemoteClient:
    ...
    async def _request(self, method: str, suffix: str, body: dict=None, timeout=None):
        ...
        try:
            response = await self._client().request(method, path, content=raw if body is not None else None, headers=headers, timeout=timeout or DEFAULT_TIMEOUT)
    ...
    async def create_session(self, url: str, institution: str, username: str, password: str):
        return await self._request('POST', '/sessions', {'url': url, 'institution': institution, 'username': username, 'password': password})
    ...
    async def list_courses(self, session_id: str, bond_id: str):
        return await self._request('GET', f'/sessions/{session_id}/bonds/{bond_id}/courses', timeout=DETAILS_TIMEOUT)

    async def course_details(self, session_id: str, bond_id: str, course_id: int):
        return await self._request('GET', f'/sessions/{session_id}/bonds/{bond_id}/courses/{course_id}/details', timeout=DETAILS_TIMEOUT)

    async def history(self, session_id: str, bond_id: str, cached_history=None, parallel=True):
        return await self._request('POST', f'/sessions/{session_id}/bonds/{bond_id}/history', {'cached_history': cached_history, 'parallel': parallel}, timeout=HISTORY_TIMEOUT)

    async def enrollment_disciplines(self, session_id: str, bond_id: str):
        return await self._request('GET', f'/sessions/{session_id}/bonds/{bond_id}/enrollment', timeout=DETAILS_TIMEOUT)

    async def enrollment_selection(self, session_id: str, bond_id: str, selected_class_ids):
        return await self._request('POST', f'/sessions/{session_id}/bonds/{bond_id}/enrollment/selection', {'selected_class_ids': [str(c) for c in selected_class_ids]}, timeout=DETAILS_TIMEOUT)

    async def enrollment_confirm(self, session_id: str, bond_id: str, password: str):
        return await self._request('POST', f'/sessions/{session_id}/bonds/{bond_id}/enrollment/confirm', {'password': password}, timeout=DETAILS_TIMEOUT)
