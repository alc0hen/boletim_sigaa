...
def create_app():
    ...
    async def set_security_headers(response):
        ...
        response.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.jsdelivr.net; font-src 'self' https://fonts.gstatic.com https://cdn.jsdelivr.net; img-src 'self' data: https://lh3.googleusercontent.com; connect-src 'self'; base-uri 'self'; form-action 'self'; object-src 'none'; frame-ancestors 'self'"
    ...
    async def log_request_start():
        ...
        http_log.info(format_http_start(req.method, req.path, '-' if is_anonymous_route else req.remote_addr or '-'))
    ...
    async def log_request_end(response):
        ...
        http_log.info(format_http_end(req.method, req.path, response.status_code, elapsed, user_id))
        return response
    ...
    async def handle_unhandled_exception(exc):
        ...
        if isinstance(exc, HTTPException):
            return exc
        http_log.error(f'❌ Exceção não tratada em {req.method} {req.path}\n' + traceback.format_exc())
